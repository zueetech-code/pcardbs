...
export function AssignClientsDialog({ open, onOpenChange, engineer, clients, onSuccess }: AssignClientsDialogProps) {
 ...
 const handleSave = async () => {
  ...
  try {
    const res = await fetch(`/api/engineers/${engineer.uid}/assign-clients`, {
      method: "PATCH",
      headers: {
    ...
    if (!res.ok) {
      throw new Error(data.error || "Failed to assign clients")
    }
            ...
            <div className="space-y-3">
              {clients.length === 0 ? (
                ...
                clients.map((client, index) => (
                  <div key={client.id + '-' + index} className="flex items-start space-x-3 rounded-lg border p-3 hover:bg-accent/50">
                    <Checkbox
